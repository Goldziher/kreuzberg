...
#### `src/extractors/` and `src/extraction/` - Separation of Concerns

Kreuzberg separates extraction into two architectural layers:

- **`src/extractors/`**: Plugin implementations that integrate with the registry system
- **`src/extraction/`**: Format-specific parsing logic and utilities

**Why separate?**

This separation follows the **Interface Segregation** and **Single Responsibility** principles:

1. **Plugin Interface Layer** (`extractors/`): Thin adapters that implement the `DocumentExtractor` trait, register MIME types, and coordinate the extraction flow
2. **Parsing Implementation Layer** (`extraction/`): Reusable parsing functions that handle the complex format-specific logic

**Benefits**:

- Parsing logic can be tested independently of the plugin system
- Extraction functions can be called directly without going through the trait interface
- Plugin implementations stay focused on orchestration and configuration
- Implementation details remain decoupled from the public plugin API

**Example formats**: PDF, Excel, PowerPoint, Email, HTML, XML, Archives

For format-specific details, see the [Extractors Guide](extractors.md).
