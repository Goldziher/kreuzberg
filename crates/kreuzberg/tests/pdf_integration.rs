...
fn test_pdf_with_code_and_formulas() {
    ...
    assert!(result.metadata.pdf.is_some() || !result.metadata.additional.is_empty());
}
...
fn test_pdf_metadata_extraction() {
    ...
    assert!(
        result.metadata.pdf.is_some() || !result.metadata.additional.is_empty(),
        "PDF should have metadata (either pdf struct or additional fields)"
    );
}
