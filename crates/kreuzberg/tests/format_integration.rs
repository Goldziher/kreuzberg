...
async fn test_pdf_simple_text_extraction() {
    if !test_documents_available() {
        tracing::debug!("Skipping test: test_documents not available");
        return;
    }

    let path = get_test_file_path("pdfs/code_and_formula.pdf");
    if !path.exists() {
        tracing::debug!("Skipping test: file not found");
        return;
    }
...
async fn test_pdf_large_document() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: large PDF not available");
        return;
    }
...
async fn test_pdf_password_protected() {
    ...
    match result {
        Err(e) => {
            tracing::debug!("Password protection detected (expected): {}", e);
        }
        Ok(res) => {
            tracing::debug!("Protected PDF extracted (some protection can be bypassed)");
...
async fn test_pdf_metadata_extraction() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: PDF not available");
        return;
    }
...
async fn test_pdf_with_tables() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: PDF with tables not available");
        return;
    }
    ...
    if !result.tables.is_empty() {
        tracing::debug!("Tables extracted: {}", result.tables.len());
...
async fn test_docx_basic_extraction() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: DOCX not available");
        return;
    }
...
async fn test_xlsx_basic_extraction() {
    ...
    if !path.exists() {
        ...
        if !path.exists() {
            tracing::debug!("Skipping test: XLSX not available");
            return;
        }
    }
    ...
    if has_sheet_info {
        tracing::debug!("Sheet metadata found (good!)");
    }
...
async fn test_pptx_basic_extraction() {
    ...
    tracing::debug!("Skipping test: No PPTX files available");
}
...
async fn test_ocr_simple_english() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: OCR test image not available");
        return;
    }
    ...
    match result {
        Ok(res) => {
            ...
            if has_expected_text {
                tracing::debug!("OCR successfully extracted expected text");
            } else {
                tracing::debug!("OCR extracted text but may not be perfect: {}", res.content);
            }
        }
        Err(e) => {
            tracing::debug!("OCR test failed (Tesseract may not be installed): {}", e);
        }
    }
}
...
async fn test_ocr_no_text_image() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: flower image not available");
        return;
    }
...
async fn test_image_without_ocr() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: example image not available");
        return;
    }
    ...
    if has_image_metadata {
        tracing::debug!("Image metadata extracted without OCR (good!)");
    }
}
...
async fn test_html_to_markdown() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: HTML file not available");
        return;
    }
    ...
    if has_markdown_patterns {
        tracing::debug!("HTML successfully converted to Markdown");
    }
...
async fn test_html_complex_layout() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: Wikipedia HTML not available");
        return;
    }
...
async fn test_html_non_english() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: German HTML not available");
        return;
    }
...
async fn test_markdown_metadata_extraction() {
    ...
    if !path.exists() {
        tracing::debug!("Skipping test: Markdown file not available");
        return;
    }
    ...
    if has_markdown_metadata {
        tracing::debug!("Markdown metadata successfully extracted");
    }
}
...
async fn test_plain_text_extraction() {
    ...
    tracing::debug!("Skipping test: No plain text files available");
}
...
async fn test_json_extraction() {
    ...
    if !path.exists() {
        ...
        if !path.exists() {
            tracing::debug!("Skipping test: JSON file not available");
            return;
        }
    }
...
async fn test_yaml_extraction() {
    ...
    if !path.exists() {
        ...
        if !path.exists() {
            tracing::debug!("Skipping test: YAML file not available");
            return;
        }
    }
...
async fn test_xml_extraction() {
    ...
    if !xml_dir.exists() {
        tracing::debug!("Skipping test: xml directory not available");
        return;
    }
    ...
    if xml_files.is_empty() {
        tracing::debug!("Skipping test: No XML files found");
        return;
    }
    ...
    if has_xml_metadata {
        tracing::debug!("XML metadata successfully extracted");
    }
...
async fn test_email_extraction() {
    ...
    if !email_dir.exists() {
        tracing::debug!("Skipping test: email directory not available");
        return;
    }
    ...
    if eml_files.is_empty() {
        tracing::debug!("Skipping test: No EML files found");
        return;
    }
    ...
    if has_email_metadata {
        tracing::debug!("Email metadata successfully extracted");
    }
