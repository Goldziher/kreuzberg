...
def extract_tables_sync(
    ...
    try:
        ...
        doc = run_sync(PyPDFium2Document, str(file_path))
        cropped_tables: list[CroppedTable] = []
        dataframes: list[DataFrame] = []
        try:
            for page in doc:
                cropped_tables.extend(run_sync(detector.extract, page))

            for cropped_table in cropped_tables:
                formatted_table = run_sync(formatter.extract, cropped_table)
                dataframes.append(run_sync(formatted_table.df))
