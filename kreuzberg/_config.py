...
# V3 config fields that indicate old config format
_V3_CONFIG_FIELDS = {
    "ocr_backend",
    "chunk_content",
    "extract_tables",
...
def _check_for_v3_config(config_dict: dict[str, Any]) -> None:
    """Check if config dict contains V3 patterns and raise helpful error."""
...
def build_extraction_config_from_dict(config_dict: dict[str, Any]) -> ExtractionConfig:
    """Build ExtractionConfig from V4 TOML dict.

    Raises ValidationError if V3 config format is detected.
    """
...
def find_config_file(start_path: Path | None = None) -> Path | None:
    """Find kreuzberg.toml or pyproject.toml with [tool.kreuzberg] section."""
...
def load_default_config(start_path: Path | None = None) -> ExtractionConfig | None:
    """Load config from kreuzberg.toml or pyproject.toml, if found."""
...
def discover_and_load_config(start_path: Path | str | None = None) -> ExtractionConfig:
    """Discover and load config, raising error if not found."""
...
def discover_config(start_path: Path | str | None = None) -> ExtractionConfig | None:
    """Discover and load config, returning None if not found."""
    search_path = Path(start_path) if start_path else None
    config_path = find_config_file(search_path)
