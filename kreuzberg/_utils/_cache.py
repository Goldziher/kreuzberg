...
class KreuzbergCache(Generic[T]):
    """Generic cache wrapper around Rust GenericCache.

    Provides a thin Python wrapper with msgpack serialization/deserialization.
    Processing coordination via threading.Event for backward compatibility.
    """
    ...
    def set(self, cache_key: str, value: T, source_file: str | None = None) -> None:
        """Set cached value by cache key (accepts raw bytes)."""
        try:
            # value should be bytes already
    ...
    def is_processing(self, cache_key: str) -> bool:
        """Check if a cache key is being processed."""
        with self._lock:
    ...
    def mark_processing(self, cache_key: str) -> threading.Event:
        """Mark a cache key as being processed, returns Event for coordination."""
        with self._lock:
    ...
    def mark_complete(self, cache_key: str) -> None:
        """Mark a cache key as processing complete."""
        with self._lock:
    ...
    def clear(self) -> tuple[int, float]:
        """Clear all cached results, returns (files_removed, mb_freed)."""
...
# Lazy cache instances using Ref pattern
