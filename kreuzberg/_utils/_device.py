...
def validate_device(requested: DeviceType, backend: str) -> str:
    ...
    if requested not in available_devices:
        raise DeviceNotAvailableError(
            f"Requested device '{requested}' not available for {backend}. "
...
def get_device_memory_info(device: str) -> Optional[dict]:
    ...
    Returns:
        Optional[dict]: Memory info dict or None if not available
    """
...
def set_memory_limit(device: str, limit_gb: float) -> None:
    ...
    if device == "cuda" and torch.cuda.is_available():
        ...
        logger.info(f"Set CUDA memory limit to {limit_gb}GB") 