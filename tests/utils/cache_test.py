...
Tests the REAL production API, not legacy test-only APIs.
"""
...
# KreuzbergCache tests (low-level cache_key API)
def test_kreuzberg_cache_init(temp_cache_dir: Path) -> None:
    ...
    assert cache.cache_dir == temp_cache_dir / "test"  # GenericCache appends cache_type
...
def test_kreuzberg_cache_source_file_tracking(cache: KreuzbergCache[bytes], temp_cache_dir: Path) -> None:
    ...
    # Modify source file (change size)
...
# DocumentCache tests (high-level domain API)
@pytest.fixture
...
# Module-level cache instance tests
