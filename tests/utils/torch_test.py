"""Tests for centralized PyTorch utilities."""
...
class TestTorchAvailability:
    ...
    def test_is_torch_available_when_torch_imported(self) -> None:
        """Test torch availability when torch is available."""
        # This will be True in our test environment since torch is installed
    ...
    def test_is_torch_available_when_torch_missing(self) -> None:
        """Test torch availability when torch is None."""
    ...
    def test_get_torch_version_available(self) -> None:
        """Test getting torch version when available."""
        mock_torch = Mock()
...
class TestRequireTorch:
    ...
    def test_require_torch_missing_default_group(self) -> None:
        ...
        with patch("kreuzberg._utils._torch.torch", None):
            ...
            assert "gmft" in str(error)  # default group
            assert "torch" in str(error)

    def test_require_torch_missing_custom_group(self) -> None:
        """Test require_torch when torch is missing with custom group."""
        with patch("kreuzberg._utils._torch.torch", None):
            with pytest.raises(MissingDependencyError) as exc_info:
...
class TestCudaUtilities:
    ...
    def test_is_cuda_available_cuda_false(self) -> None:
        """Test CUDA availability when CUDA is not available."""
        mock_torch = Mock()
    ...
    def test_get_cuda_device_count_torch_missing(self) -> None:
        """Test CUDA device count when torch is missing."""
        with patch("kreuzberg._utils._torch.torch", None):
            assert get_cuda_device_count() == 0

    def test_get_cuda_device_count_cuda_unavailable(self) -> None:
        """Test CUDA device count when CUDA is unavailable."""
        mock_torch = Mock()
        mock_torch.cuda.is_available.return_value = False
        with patch("kreuzberg._utils._torch.torch", mock_torch):
            assert get_cuda_device_count() == 0
    ...
    def test_get_cuda_device_count_cuda_available(self) -> None:
        """Test CUDA device count when CUDA is available."""
        mock_torch = Mock()
        mock_torch.cuda.is_available.return_value = True
    ...
    def test_get_cuda_device_properties_success(self) -> None:
        """Test CUDA device properties when successful."""
        mock_torch = Mock()
        mock_torch.cuda.is_available.return_value = True
        ...
        with patch("kreuzberg._utils._torch.torch", mock_torch):
            ...
            assert props["total_memory"] == 10.0  # GB
    ...
    def test_get_cuda_device_properties_exception(self) -> None:
        """Test CUDA device properties when exception occurs."""
        mock_torch = Mock()
        mock_torch.cuda.is_available.return_value = True
    ...
    def test_get_cuda_memory_info_success(self) -> None:
        ...
        with patch("kreuzberg._utils._torch.torch", mock_torch):
            ...
            assert total == 8.0  # 8 GB total
            assert available == 6.0  # 6 GB available (8 - 2)

    def test_get_cuda_memory_info_exception(self) -> None:
        """Test CUDA memory info when exception occurs."""
        mock_torch = Mock()
        mock_torch.cuda.is_available.return_value = True
...
class TestMpsUtilities:
    ...
    def test_is_mps_available_mps_false(self) -> None:
        """Test MPS availability when MPS is not available."""
        mock_torch = Mock()
    ...
    def test_is_mps_available_old_pytorch(self) -> None:
        """Test MPS availability with older PyTorch that lacks MPS."""
        mock_torch = Mock()
        del mock_torch.backends.mps  # Simulate older PyTorch
        with patch("kreuzberg._utils._torch.torch", mock_torch):
            assert is_mps_available() is False
...
class TestDeviceResolution:
    ...
    def test_resolve_device_explicit_mps(self) -> None:
        """Test device resolution with explicit MPS."""
    ...
    def test_resolve_device_auto_mps_available(self) -> None:
        """Test auto device resolution when MPS is available (no CUDA)."""
        mock_torch = Mock()
        mock_torch.cuda.is_available.return_value = False
        mock_torch.backends.mps.is_available.return_value = True
        with patch("kreuzberg._utils._torch.torch", mock_torch):
    ...
    def test_resolve_device_auto_cpu_only(self) -> None:
        """Test auto device resolution when only CPU available."""
        with patch("kreuzberg._utils._torch.torch", None):
...
class TestGpuCaching:
    ...
    def test_clear_gpu_cache_torch_missing(self) -> None:
        """Test GPU cache clearing when torch is missing."""
        with patch("kreuzberg._utils._torch.torch", None):
            # Should not raise
            clear_gpu_cache()
    ...
    def test_clear_gpu_cache_mps_available(self) -> None:
        """Test GPU cache clearing when MPS is available."""
        mock_torch = Mock()
        mock_torch.cuda.is_available.return_value = False
        mock_torch.backends.mps.is_available.return_value = True
    ...
    def test_clear_gpu_cache_both_available(self) -> None:
        """Test GPU cache clearing when both CUDA and MPS are available."""
        mock_torch = Mock()
        mock_torch.cuda.is_available.return_value = True
        mock_torch.backends.mps.is_available.return_value = True
    ...
    def test_clear_gpu_cache_cuda_exception(self) -> None:
        """Test GPU cache clearing when CUDA raises exception."""
        mock_torch = Mock()
        mock_torch.cuda.is_available.return_value = True
    ...
    def test_clear_gpu_cache_mps_exception(self) -> None:
        """Test GPU cache clearing when MPS raises exception."""
        mock_torch = Mock()
        mock_torch.cuda.is_available.return_value = False
        mock_torch.backends.mps.is_available.return_value = True
        ...
        with patch("kreuzberg._utils._torch.torch", mock_torch):
            # Should not raise despite MPS error
            clear_gpu_cache()


class TestTensorOperations:
    """Test tensor creation and gradient context."""

    def test_tensor_torch_available(self) -> None:
        """Test tensor creation when torch is available."""
        mock_torch = Mock()
        mock_tensor = Mock()
    ...
    def test_tensor_torch_missing(self) -> None:
        """Test tensor creation when torch is missing."""
        with patch("kreuzberg._utils._torch.torch", None):
            with pytest.raises(MissingDependencyError) as exc_info:
                tensor([1, 2, 3])
    ...
    def test_with_no_grad_torch_available(self) -> None:
        """Test gradient context when torch is available."""
        mock_torch = Mock()
        mock_context = Mock()
    ...
    def test_with_no_grad_torch_missing(self) -> None:
        """Test gradient context when torch is missing (should return nullcontext)."""
        with patch("kreuzberg._utils._torch.torch", None):
            context = with_no_grad()

            # Should be a nullcontext that works as a no-op
            with context:
                pass  # Should not raise


class TestLogging:
    """Test torch information logging."""

    def test_log_torch_info_torch_missing(self, caplog: pytest.LogCaptureFixture) -> None:
        """Test logging when torch is missing."""
    ...
    def test_log_torch_info_torch_available_no_gpu(self, caplog: pytest.LogCaptureFixture) -> None:
        """Test logging when torch is available but no GPU."""
        mock_torch = Mock()
        mock_torch.__version__ = "2.0.0"
        mock_torch.cuda.is_available.return_value = False
        mock_torch.backends.mps.is_available.return_value = False
    ...
    def test_log_torch_info_torch_available_with_cuda(self, caplog: pytest.LogCaptureFixture) -> None:
        """Test logging when torch is available with CUDA."""
        mock_torch = Mock()
        mock_torch.__version__ = "2.0.0"
        mock_torch.cuda.is_available.return_value = True
        mock_torch.cuda.device_count.return_value = 2
    ...
    def test_log_torch_info_torch_available_with_mps(self, caplog: pytest.LogCaptureFixture) -> None:
        """Test logging when torch is available with MPS."""
        mock_torch = Mock()
        mock_torch.__version__ = "2.0.0"
        mock_torch.cuda.is_available.return_value = False
        mock_torch.backends.mps.is_available.return_value = True
...
class TestEdgeCases:
    ...
    def test_torch_import_in_type_checking_mode(self) -> None:
        """Test that torch utilities work correctly during type checking."""
        # This test ensures our TYPE_CHECKING import pattern works
        from kreuzberg._utils._torch import is_torch_available

        # Should work regardless of TYPE_CHECKING mode
    ...
    def test_torch_none_vs_import_error(self, mocker: MockerFixture) -> None:
        """Test difference between torch=None and import error."""
        # Test when torch is explicitly None
        with patch("kreuzberg._utils._torch.torch", None):
            assert is_torch_available() is False

        # Test when torch import would fail (simulate cold import)
        ...
        # Import should handle the ImportError gracefully
        # Note: This is testing the initial import behavior, not runtime behavior
        from kreuzberg._utils import _torch

        # The module should still be importable even if torch isn't available
