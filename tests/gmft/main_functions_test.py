"""Tests for main GMFT module functions and integration."""
...
def test_extract_tables_sync_with_default_config() -> None:
    ...
    test_pdf = "tests/test_source_files/gmft/tiny.pdf"
...
def test_cached_detector_different_configs() -> None:
    # Test that different configs return different detector instances
    ...
    detector3 = _get_cached_detector("microsoft/table-transformer-detection", 0.7)  # Same as first

    # Different configs should give different instances
    assert detector1 is not detector2
    # Same config should give same instance (cached)
    assert detector1 is detector3


def test_cached_formatter_different_configs() -> None:
    # Test that different configs return different formatter instances
    ...
    formatter3 = _get_cached_formatter(
        "microsoft/table-transformer-structure-recognition-v1.1-all", 0.5
    )  # Same as first
...
def test_config_hash_generation() -> None:
    # Test that config hash is generated properly for caching
    ...
    # Different configs should generate different hashes
