...
def test_html_invalid_base64_handling() -> None:
    html_content = """
    <html>
    <body>
...
def test_html_malformed_svg_handling() -> None:
    html_content = """
    <html>
    <body>
        <svg>Incomplete SVG
...
def test_pdf_corrupted_image_stream_sync(monkeypatch: pytest.MonkeyPatch) -> None:
    extractor = PDFExtractor(mime_type="application/pdf", config=ExtractionConfig(images=ImageExtractionConfig()))
...
def test_presentation_with_real_file(pptx_document: Path) -> None:
    config = ExtractionConfig(images=ImageExtractionConfig())
    ...
    with pytest.raises(OSError, match="PPTX extraction failed"):
        extractor._extract_from_bytes(b"invalid_pptx_data")


@pytest.mark.anyio
async def test_ocr_backend_not_available() -> None:
...
@pytest.mark.anyio
async def test_ocr_processing_exception() -> None:
...
def test_empty_image_data() -> None:
    config = ExtractionConfig(images=ImageExtractionConfig())
    extractor = PDFExtractor(mime_type="application/pdf", config=config)
...
def test_image_with_none_fields() -> None:
    img = ExtractedImage(
        data=b"test",
        format="png",
        filename=None,
...
def test_concurrent_extraction_errors_sync() -> None:
    extractor = PDFExtractor(mime_type="application/pdf", config=ExtractionConfig(images=ImageExtractionConfig()))
...
def test_extract_images_disabled(pptx_document: Path) -> None:
    config = ExtractionConfig(images=None)
