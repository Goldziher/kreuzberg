...
from kreuzberg._types import ExtractionResult, ImageExtractionConfig, ImageOCRResult, TesseractConfig
...
def test_extract_images_from_playa_sync_single_image(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_extract_images_from_playa_sync_multiple_pages(monkeypatch: pytest.MonkeyPatch) -> None:
    extractor = PDFExtractor(mime_type="application/pdf", config=ExtractionConfig())
...
def test_extract_images_from_playa_sync_error_handling(monkeypatch: pytest.MonkeyPatch, caplog: Any) -> None:
    extractor = PDFExtractor(mime_type="application/pdf", config=ExtractionConfig())
...
def test_extract_path_sync_with_images(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_extract_path_sync_with_memory_limits(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    pdf_path = tmp_path / "test.pdf"
    pdf_path.write_bytes(b"%PDF-1.4\n%%EOF")
...
def test_extract_path_sync_with_ocr(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    pdf_path = tmp_path / "test.pdf"
    pdf_path.write_bytes(b"%PDF-1.4\n%%EOF")
