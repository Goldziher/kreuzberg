//! Streaming text and markdown parsing with metadata extraction
//!
//! This module provides memory-efficient text parsing that can handle
//! very large text files through line-by-line streaming.

use once_cell::sync::Lazy;
use pyo3::prelude::*;
use regex::Regex;

use crate::string_utils::safe_decode;
...
/// Result of text/markdown extraction
#[pyclass]
pub struct TextExtractionResult {
    ...
    /// Markdown headers (None for plain text)
    #[pyo3(get)]
    pub headers: Option<Vec<String>>,
    /// Markdown links as (text, url) tuples (None for plain text)
    #[pyo3(get)]
    pub links: Option<Vec<(String, String)>>,
    /// Code blocks as (language, code) tuples (None for plain text)
    #[pyo3(get)]
...
impl TextExtractionResult {
    fn __repr__(&self) -> String {
        format!(
            "TextExtractionResult(lines={}, words={}, chars={}, headers={}, links={}, code_blocks={})",
            self.line_count,
            self.word_count,
            self.character_count,
            self.headers.as_ref().map(|h| h.len()).unwrap_or(0),
            self.links.as_ref().map(|l| l.len()).unwrap_or(0),
            self.code_blocks.as_ref().map(|c| c.len()).unwrap_or(0)
        )
    }
}

/// Parse text or markdown from bytes using streaming parser
///
/// This function processes text line-by-line for memory efficiency:
/// - Memory usage is O(max_line_size) not O(file_size)
/// - Can handle multi-GB text files
/// - For markdown: extracts headers, links, and code blocks
///
/// # Arguments
/// * `text_bytes` - Text content as bytes
/// * `is_markdown` - Whether to extract markdown-specific metadata
///
/// # Returns
/// TextExtractionResult with content and metadata
///
/// # Errors
/// Returns PyErr if text decoding fails
#[pyfunction]
pub fn parse_text(py: Python<'_>, text_bytes: &[u8], is_markdown: bool) -> PyResult<TextExtractionResult> {
    py.detach(|| parse_text_internal(text_bytes, is_markdown))
}

fn parse_text_internal(text_bytes: &[u8], is_markdown: bool) -> PyResult<TextExtractionResult> {
    // Decode bytes to text using safe_decode
    let text = safe_decode(text_bytes, None);
    ...
    for line in text.lines() {
        ...
        // Extract headers (only outside code blocks)
        ...
        // Extract links (only outside code blocks)
...
pub fn register_text_functions(m: &Bound<'_, PyModule>) -> PyResult<()> {
...
mod tests {
    ...
    fn test_plain_text_basic() {
        ...
        let result = parse_text_internal(text, false).unwrap();
    ...
    fn test_markdown_headers() {
        let text = b"# Header 1\n## Header 2\n### Header 3\n#NoSpace\n## Multiple  spaces";
        let result = parse_text_internal(text, true).unwrap();
    ...
    fn test_large_text_streaming() {
        let large_text = "Line\n".repeat(10000);
        let result = parse_text_internal(large_text.as_bytes(), false).unwrap();
