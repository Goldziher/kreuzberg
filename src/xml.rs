...
//!
//! This module provides high-performance streaming XML parsing that can handle
//! multi-GB XML files without loading the entire document into memory.
...
/// Result of XML extraction
#[pyclass]
pub struct XmlExtractionResult {
    /// Extracted text content from all elements
    #[pyo3(get)]
    pub content: String,
...
/// Parse XML from bytes using streaming parser
///
/// This function uses quick-xml's event-based streaming parser to extract:
/// - Text content from all elements
/// - Element count and unique element names
///
/// Memory usage is O(output_size + unique_elements) not O(input_size), making it suitable
/// for very large XML files.
///
/// # Arguments
/// * `xml_bytes` - XML content as bytes
/// * `preserve_whitespace` - Whether to preserve whitespace in text content
///
/// # Returns
/// XmlExtractionResult with extracted content and metadata
///
/// # Errors
/// Returns PyErr if XML parsing fails critically (e.g., invalid UTF-8 in text content)
#[pyfunction]
